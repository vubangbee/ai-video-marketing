...
Use Nano Banana image-to-image with character portrait as `--ref` reference:
```bash
npx -y -p @mrgoonie/multix@0.2.0 multix gemini image-to-image \
  --ref assets/storyboards/{slug}/characters/{character_name}-portrait.png \
  ...
  --output assets/storyboards/{slug}/scene-{N}-1-start.png
```
...
**DO NOT** use the default read_file capability to read the frames unless `ai-multimodal` skill fails.
