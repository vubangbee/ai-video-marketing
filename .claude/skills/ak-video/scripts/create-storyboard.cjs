...
const MULTIX_COMMAND = ['npx', '-y', '-p', '@mrgoonie/multix@0.2.0', 'multix'];
...
function checkAiMultimodal() {
  try {
    execFileSync(MULTIX_COMMAND[0], MULTIX_COMMAND.slice(1).concat(['--version']), {
      encoding: 'utf8',
    ...
    if (errMsg.includes('node')) {
      return { available: false, reason: 'Node.js 20+ or npx unavailable' };
    }
    return { available: false, reason: 'multix unavailable' };
  }
}
...
async function generateFrame(prompt, outputPath, aspectRatio, skipGeneration = false) {
  ...
  try {
    ...
    const result = execFileSync(MULTIX_COMMAND[0], MULTIX_COMMAND.slice(1).concat([
      'gemini',
      'generate',
      '--prompt',
      prompt,
    ...
    return { success: false, prompt, error: `Could not locate generated image. Output: ${result}` };
  } catch (error) {
